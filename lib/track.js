...
const { trackRoot, apiRoot, Regions } = require('./common');
...
module.exports = class TrackClient {
  constructor(siteid, apikey, defaults = { region: Regions.us }) {
    this.siteid = siteid;
    this.apikey = apikey;
    this.defaults = defaults;
    this.request = new Request({ siteid: this.siteid, apikey: this.apikey }, this.defaults);

    if (!Object.values(Regions).includes(defaults.region)) {
      throw new Error('region must be one of "eu" or "us"');
    }

    this.trackRoot = defaults && defaults.url ? defaults.url : trackRoot[defaults.region];
    this.apiRoot = defaults && defaults.apiUrl ? defaults.apiUrl : apiRoot[defaults.region];
  }
