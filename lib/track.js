...
module.exports = class TrackClient {
  ...
  addToSegment(segmentId, customerIds = []) {
    ...
    if (isEmpty(customerIds) || customerIds.length === 0) {
      throw new MissingParamError('customerIds');
    }

    return this.request.post(`${this._trackRoot}/segments/${encodeURIComponent(segmentId)}/add_customers`, {
      ids: customerIds,
    });
  }
  ...
  removeFromSegment(segmentId, customerIds = []) {
    ...
    return this.request.post(`${this._trackRoot}/segments/${encodeURIComponent(segmentId)}/remove_customers`, {
      ids: customerIds,
    });
  }
};
