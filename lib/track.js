...
module.exports = class TrackClient {
  constructor(siteid, apikey, defaults = { region: 'us' }) {
    this.siteid = siteid;
    this.apikey = apikey;
    this.defaults = defaults;
    this.request = new Request({ siteid: this.siteid, apikey: this.apikey }, this.defaults);

    if (defaults.region && !['us', 'eu'].includes(defaults.region)) {
      throw new Error('region must be one of "eu" or "us"');
    }

    this._trackRoot = defaults && defaults.url ? defaults.url : trackRoot[defaults.region];
    this._apiRoot = defaults && defaults.apiUrl ? defaults.apiUrl : apiRoot[defaults.region];
  }
